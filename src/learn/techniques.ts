...
export const techniques: TechniqueArticle[] = [
  ...
  },
  {
    ...
    difficulty: 'Advanced',
    summary: 'Follow chains of strong links on a single candidate and assign alternating colors. If two same-color cells see each other, that color is invalid.',
    example: 'Candidate 5 appears in exactly two cells in row 3 (r3c2, r3c8) and exactly two cells in column 2 (r3c2, r7c2). These form a chain: r3c2 — r3c8 — r5c8 — r5c4 — r7c4 — r7c2 — r3c2. Assign alternating colors A and B. If r3c2 is color A, then r3c8 is B, r5c8 is A, r5c4 is B, r7c4 is A, r7c2 is B. Now r3c2 (A) and r5c8 (A) are both color A — and they see each other (same column). Two same-color cells cannot both be true, so color A is impossible. Remove 5 from all color-A cells.',
    exampleGrid: '070000043040009610800634900094052000358460020000800530080070091902100005007040802',
    exampleTechniqueName: 'Simple Coloring',
    whenToLook: [
      'When a candidate appears in exactly two cells in multiple houses, creating linked pairs.',
      'When you can trace a path of "either-or" relationships on a single digit.',
    ],
    steps: [
      'Pick a candidate that forms strong links (appears in exactly two cells) in multiple houses.',
      'Follow the chain, assigning alternating colors (A/B) to each cell.',
      'If two cells of the same color see each other, that color is invalid — remove the candidate from all same-color cells.',
      'If a cell outside the chain sees both a color-A and color-B cell, remove the candidate from that cell.',
    ],
    mistakes: [
      'Following weak links (candidate appears in 3+ cells) — only strong links (exactly 2 cells) create valid color alternation.',
      'Removing the candidate from all cells of the wrong color — the conflict determines which color is invalid.',
      'Confusing Simple Coloring with XY-Wing — coloring operates on a single digit across the whole grid, XY-Wing uses three bivalue cells.',
    ],
  },
  {
    slug: 'skyscraper',
    title: 'Skyscraper',
    difficulty: 'Advanced',
    summary: 'Two strong links on the same digit share a base. The two tops see different cells — remove the digit from any cell seeing both tops.',
    example: 'Candidate 3 appears in exactly two cells in row 2 (r2c1, r2c7) and exactly two cells in row 8 (r8c1, r8c5). r2c1 and r8c1 share column 1 — these are the bases. r2c7 and r8c5 are the tops. One base must be 3, so one top must also be 3. Any cell that sees both r2c7 and r8c5 cannot contain 3.',
    exampleGrid: '070000043040009610800634900094052000358460020000800530080070091902100005007040802',
    exampleTechniqueName: 'Skyscraper',
    whenToLook: [
      'When two strong links on the same digit exist in parallel houses (two rows or two columns).',
      'When one cell from each link shares a house, creating a "base" connection.',
    ],
    steps: [
      'Find two strong links on the same digit in the same house type (both rows or both columns).',
      'Identify the base: one cell from each link that shares a house.',
      'The other two cells are the tops — they must not share a house (otherwise it is a fish pattern).',
      'Remove the digit from any cell that sees both tops.',
    ],
    mistakes: [
      'Confusing with X-Wing — if the tops also share a house, it is a fish pattern, not a Skyscraper.',
      'Eliminating from cells that see only one top — the target must see both.',
      'Using links from different house types (one row, one column) — both must be the same type.',
    ],
  },
  {
    slug: 'w-wing',
    title: 'W-Wing',
    difficulty: 'Advanced',
    summary: 'Two bivalue cells share a candidate X. A strong link on X connects them. Remove X from any cell seeing both bivalue cells.',
    example: 'r1c3 has candidates {2, 6}. r5c7 has candidates {2, 6}. Both share candidate 2. A strong link on 2 exists between r1c5 and r5c5 (column 5 has exactly two cells with candidate 2). r1c3 sees r1c5 (same row) and r5c7 sees r5c5 (same row). If r1c3 is not 2, then r1c5 must be 2 (strong link), forcing r5c5 to not be 2, which forces r5c7 to be 2. Either way, one of the two bivalue cells must be 2. Remove 2 from any cell that sees both r1c3 and r5c7.',
    exampleGrid: '070000043040009610800634900094052000358460020000800530080070091902100005007040802',
    exampleTechniqueName: 'W-Wing',
    whenToLook: [
      'When two bivalue cells share a candidate and a strong link exists on that candidate.',
      'When a bivalue cell sees one end of a strong link and another bivalue cell sees the other end.',
    ],
    steps: [
      'Find two bivalue cells that share a candidate X.',
      'Find a strong link on X (two cells in the same house) where one bivalue cell sees one end and the other sees the other end.',
      'One of the two bivalue cells must contain X.',
      'Remove X from any cell that sees both bivalue cells.',
    ],
    mistakes: [
      'Using cells with more than two candidates — both cells must be bivalue.',
      'Forgetting to verify the strong link — the connecting cells must have exactly two candidates for X in their house.',
      'Eliminating from cells that see only one bivalue cell — the target must see both.',
    ],
  },
  {
